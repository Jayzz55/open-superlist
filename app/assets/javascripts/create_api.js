    ...
    $("body").on("click", "[data-create-button]", function(event) {
      ...
      $.ajax({
        ...
        success : function(data) {
          $('#new_todo_body').val("");
          insertRow = '<tbody class="tbody" id="todo-'+data.id+'"><tr><td>'+data.body+'</td><td>7</td><td><input id="todo_'+data.id+'" name="todos[]" value='+data.id+'  type= "checkbox"  /><td><button class="btn btn-danger" data-delete-button="true" data-todo-id="'+data.id+'" name="button" type="submit">Delete</button></td><tr/></tbody>';
